...
let msgAvvertenza = "";
if (crewMass > 1000) {
	msgAvvertenza = "DANGER! OVERLOAD ALERT: escape from ship now!";
} else if (crewMass > 900) {
	msgAvvertenza = "Critical Load: Over 900";
} else if (crewMass > 700) {
	msgAvvertenza = "Load is over 700";
} else if (crewMass > 500) {
	msgAvvertenza = "Ship is half loaded";
} else {
	msgAvvertenza = "Ship is under loaded";
}

console.log("ESERCIZIO 7: " + msgAvvertenza);
...
for (let i = 0; i < characters.length; i++) {
	for (let j = 0; j < femaleCharacters.length; j++) {
		if (characters[i] === femaleCharacters[j].name) {
			characters.splice(characters[i], 1);
		}
	}
}
console.log(characters.length);
